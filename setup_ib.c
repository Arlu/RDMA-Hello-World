...
int connect_between_qps()
{
    ...
    if (ibv_modify_qp(ib_res.qp, &qp_attr, IBV_QP_STATE | IBV_QP_PATH_MTU | IBV_QP_DEST_QPN | IBV_QP_RQ_PSN |
    ...
    if (ibv_modify_qp(ib_res.qp, &qp_attr, IBV_QP_STATE | IBV_QP_TIMEOUT | IBV_QP_RETRY_CNT |
...
int setup_ib()
{
    ...
    ib_res.cq = ibv_create_cq(ib_res.ctx, 2 * COUNT, NULL, NULL, 0);
    if (!ib_res.cq)
    {
    ...
    // Creates a Queue Pair:
    {
        ...
        if (ibv_modify_qp(ib_res.qp, &qp_attr, IBV_QP_STATE | IBV_QP_PKEY_INDEX | IBV_QP_PORT | IBV_QP_ACCESS_FLAGS)) {
    ...
    if (ib_res.port_info.link_layer == IBV_LINK_LAYER_INFINIBAND && !local_dest.lid)
    {
